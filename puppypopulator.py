...
#Add Shelters
shelter1 = Shelter(name = "Oakland Animal Services", address = "1101 29th Ave", city = "Oakland", state = "California", zipCode = "94601", website = "oaklandanimalservices.org", maxCapacity=10, currentOccupancy=0)
session.add(shelter1)

shelter2 = Shelter(name = "San Francisco SPCA Mission Adoption Center", address="250 Florida St", city="San Francisco", state="California", zipCode = "94103", website = "sfspca.org", maxCapacity=10, currentOccupancy=0)
session.add(shelter2)

shelter3 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", city = "San Francisco", state = "California" , zipCode = "94103", website = "http://wonderdogrescue.org", maxCapacity=10, currentOccupancy=0)
session.add(shelter3)

shelter4 = Shelter(name = "Humane Society of Alameda", address = "PO Box 1571" ,city = "Alameda" ,state = "California", zipCode = "94501", website = "hsalameda.org", maxCapacity=5, currentOccupancy=0)
session.add(shelter4)

shelter5 = Shelter(name = "Palo Alto Humane Society" ,address = "1149 Chestnut St." ,city = "Menlo Park", state = "California" ,zipCode = "94025", website = "paloaltohumane.org", maxCapacity=100, currentOccupancy=0)
...
def AssignPuppyToShelter(session):
    # sort shelters by remaining capacity
    # get shelter with most space.
    # if it has space return shelter, else return None

    emptiest_shelter =  session.query(Shelter).order_by(Shelter.currentOccupancy.desc()).first()
    space = emptiest_shelter.maxCapacity - emptiest_shelter.currentOccupancy
    if space > 0:
        return emptiest_shelter.id
    else:
        return None
